...
- [LLM Observability](documentation/observability.md) — Every LLM/embedding call is traced to file storage with full prompt, response and tokens; cost is applied on read by `scripts/llm_cost.py`. **Wiring invariant: every process making LLM calls must call `install_file_tracing()` once at startup and set `trace_context` at each unit-of-work boundary.** Read before touching any LLM call site or adding a process that makes one.
