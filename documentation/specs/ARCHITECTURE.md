...
**Step 4 — Metadata Extraction:** Rule-based first. Legal decisions from Överklagandenämnden likely follow a consistent template — date, case number, decision outcome, and category can probably be extracted via regex/pattern matching on known document structure. This should be attempted as a one-and-done effort: analyze a sample of documents, define extraction rules, validate across the corpus. Fallback to LLM-assisted extraction (Gemini Flash or Haiku) only for documents that fail rule-based parsing. Checkpoint: metadata record persisted.
