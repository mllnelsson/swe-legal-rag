...
## 2026-08-21
* **Update**: [Frontend](/frontend/overview.md) — the home page rebuilds as one centred box: the wordmark is its own heading, the **Sök / Agent** choice is a keyboard-reachable `Switch` rather than a `Tabs` pill, and `AppShell` drops its background, hairline and wordmark on `/` alone. The home-to-agent handover moves from a `?q=` query parameter to router state, shared with the agent composer through a `view-transition-name` so the box travels into place instead of the page swapping; the routes table and a new "Arriving from the home page" section reflect it. The agent page's earlier-conversations rail moves from a column beside the transcript into a slide-over panel behind "Tidigare samtal", with "Nytt samtal" staying always visible in a page action row — recorded in "Earlier conversations" and in [small screens](/frontend/overview.md#small-screens), which no longer counts agent mode among the two-column pages. A reopened conversation whose only turn never completed now gets its own documented state, distinct from a brand-new conversation, cross-linked to [sessions](/data-model/sessions.md#a-row-exists-before-the-conversation-does). A new "Watching the agent work" section covers the progress-step card, its elapsed counter and its fold into a `<details>` summary once the answer starts, and notes that [honesty rule 14](/frontend/honesty-rules.md) still governs the (untouched) SQL evidence block rather than the step list.
* **Update**: [Chat Endpoint](/api/chat-endpoint.md) gains "A reload mid-answer is safe", recording the two race fixes reachable by reloading while an answer streams: the session row now commits immediately rather than at request teardown, and the chat agent awaits the cancelled task driving its tool loop instead of returning while that task is still using the request-scoped database session.
* **Update**: [sessions](/data-model/sessions.md) — "A row exists before the conversation does" now states that the row is committed as soon as it is created, not left flushed-only until the turn's teardown, and why that closes a narrow reload race right after the `done` frame names a fresh conversation.
* **Update**: [Live Testing](/playbooks/live-testing.md)'s scripted-UI walkthrough table matches the rebuilt home and agent pages: the mode control is a switch with its prose underneath, the home page's keywords are caption links rather than chips, `/agent` now names the progress card and its fold plus the "Tidigare samtal" panel, and the small-screens paragraph scopes the stacking rail behavior to `/sok` and `/beslut/{id}` now that agent mode has no rail to stack.

## 2026-08-20
* **Fix**: [ai Package](/packages/ai.md) — `_load_tokenizer` now guards the `AutoTokenizer.from_pretrained` result and raises the new `TokenizerUnavailableError` naming the model. `transformers` types that call as returning `None` for a name it cannot map to a tokenizer class; unguarded the `None` reached the first `count_tokens` call and surfaced as an `AttributeError` naming neither the model nor the `embedding.model` key that chose it, after the chunk worker had already started. Found by `ty`, which had been reporting the return-type mismatch against the declared `PreTrainedTokenizerBase`.
* **Creation**: [Application Logging](/logging.md) — a top-level concept for the thing the bundle had never documented as a subject: what these processes write to stdout. Records the single `LOG_LEVEL` knob and its two-source resolution, the shared format and the DEBUG-only interaction column, the API's per-request envelope with its level table and per-endpoint field table, and the **previews-not-payloads** rule (120 characters, enforced by one helper) that keeps prompts and corpus text out of the log. Written because the API logged nothing at INFO at all — it was the only entry point in the repo that never called `configure_logging()`, so its `logger.info` calls fell through to logging's `lastResort` handler at WARNING and were discarded.
* **Update**: [LLM Observability](/observability.md) — the "Deliberately not captured" list said *"Structured application logging. Out of scope; traces are not a log"* and pointed at a document that did not exist. It now names [/logging.md](/logging.md) and states the split the two streams actually make: correlated by one `interaction_id`, logs record *flow* and carry text only as previews, traces record *payload* and carry it whole.
* **Update**: [api Package](/packages/api.md) — documents `api/logging_setup.py` and `api/access_log.py` under shared HTTP infrastructure, including why the request envelope is raw ASGI rather than `BaseHTTPMiddleware` (the latter returns at first byte, so a chat turn's exit line would time a fraction of it and carry none of its metadata) and why `add_middleware` is called with the access log *first* so CORS ends up outermost and preflight `OPTIONS` never reach the log. `api/correlation.py` gains `interaction_id_of`: the id is now resolved once per request by the middleware and read back by the chat and SQL routes, because resolving twice minted two ids for one turn. The lifespan section records the new per-stage timing lines.
* **Update**: [shared Package](/packages/shared.md) — `configure_logging(level=None)` and `resolve_log_level()`. Records why `.env` is read *directly* rather than through `load_dotenv()`: every entry point configures logging before loading dotenv, and in six of the seven workers `load_dotenv()` is inside `subscribe()` rather than `main()`, so an `os.environ`-only read would have made a `.env` `LOG_LEVEL` work under Compose and silently do nothing under `uv run`.
* **Update**: [Worker Patterns](/pipeline/worker-patterns.md) — the progress-logging section now says where verbosity comes from and links the API's equivalent envelope, so the one level table in the bundle is no longer the only place the rules are stated.
* **Update**: [Chat Endpoint](/api/chat-endpoint.md) — what one turn looks like in the server log, including the `aborted` exit line a client that navigates away mid-answer produces, which was previously invisible.
* **Update**: [Local Development Environment](/playbooks/local-dev.md) — `LOG_LEVEL` in the `.env` block and the environment-variable table, scoped `(all)` rather than `(api)`. The HuggingFace startup-stall section notes that the wait is now reported rather than silent: `Embedding provider ready in 8.4s` at DEBUG, and one `API ready in <n>s` line at INFO.

## 2026-08-19
* **Update**: [Local Development Environment](/playbooks/local-dev.md) — records why a local-embedding process appears to hang at startup and ships `HF_HUB_OFFLINE=1` in `.env`. `sentence-transformers` revalidates every cached file of `intfloat/multilingual-e5-large` against huggingface.co on load, and under WSL2 each probe waits out the 10s `HF_HUB_ETAG_TIMEOUT`, so `uvicorn` sits silent for ~90s after `Waiting for application startup.` Trusting the warm cache takes API startup to ~9s.
