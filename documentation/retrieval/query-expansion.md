...
The [frontend](/frontend/overview.md) exposes this at two entry points, off by default
and carried in the URL (`?utoka=1`) so an expanded search stays a shareable link. The
home page offers a secondary "Smart sökning" switch, shown only in search mode, that
sets `expand` on the search it navigates to. The results page offers the same choice as
a checkbox — "Sök även på omformuleringar av frågan" — inside the collapsed "Filter"
disclosure, above the narrowing controls rather than among them: expansion widens the
search rather than narrowing it, so it survives `clearFilters` and is deliberately not
counted among the active filters. When it was requested, the results summary states
whether the extra phrasings shown were generated by a model or, when
`diagnostics.expanded` came back `false`, that expansion could not be fetched and the
search ran on the query as written.
