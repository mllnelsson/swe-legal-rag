...
class TaskRepository:
    ...
    async def get_by_id(self, task_id: uuid.UUID) -> TaskRead | None:
        task = await self._session.get(Task, task_id)
        return TaskRead.model_validate(task) if task else None

    async def get_by_document_and_step(self, document_id: uuid.UUID, step: str) -> TaskRead | None:
    ...
    async def list_by_step_and_status(self, step: str, status: str) -> list[TaskRead]:
        result = await self._session.execute(
