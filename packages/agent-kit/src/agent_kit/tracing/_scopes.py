...
Lives here rather than in `llm-core` because `interaction_id` is an agent-app
...
def interaction_scope(
    ...
    what still correlates a standalone request or a script case, which have no
...
def agent_run_scope(**values: Any) -> Iterator[str]:
    ...
    calls of one tool in one turn share every other correlation key — and this
    is what gives each its own identity back.
    """
    resolved = _new_id()
