...
here rather than in llm-core so that llm-core stays a pure abstraction: it emits
`LLMCallRecord`s and leaves *where they go* to a host that opts in.

The layout carries the correlation, so nothing has to index it afterwards:

    {trace root}/{date}/{interaction_id}/{time}-{source}-{id}.json
...
# UTF-8 without escaping: prompts are frequently non-ASCII, and \uXXXX-escaping
# them can triple the size of a trace file for no benefit.
...
class LLMTraceConfig(BaseSettings):
    ...
    # The base directory the daily trace tree hangs under. Self-contained by
    # default so this package needs no host to say where traces go; a host that
    # keeps traces beside other data passes an explicit `root` instead.
    root_path: Path = Field(default=Path("."), alias="LLM_TRACE_ROOT")
...
def serialize_record(record: LLMCallRecord) -> dict[str, Any]:
    ...
    Cost is deliberately absent. `model` and `usage` are the complete raw
    material; applying a price to them is an analysis question. Writing a number
    in would only freeze a rate that may be wrong or missing, for no gain.
    """
    return {
...
def install_file_tracing(
    ...
    harmless, which is what lets one process compose several worker `main()`s.

    When `root` is given it is used as the trace root verbatim; otherwise the
    root is `config.root_path / config.directory_name`. Returns None when tracing
    is disabled or the root cannot be created — observability must never stop a
    ...
    try:
        ...
        resolved = root or (settings.root_path / settings.directory_name)
