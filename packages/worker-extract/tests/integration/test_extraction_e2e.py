...
async def _run_extraction(
    ...
    *,
    ...
    await document_repo.update(doc.id, DocumentUpdate(raw_text=raw_text, case_number=case_number))
    await session.commit()
    doc = await document_repo.get_by_id(doc.id)
    assert doc is not None

    task = await task_repo.create(
        TaskCreate(document_id=doc.id, step="extract", status="pending")
    )
    await session.commit()
...
async def test_extraction_populates_entities_and_completes_task(
    ...
    doc, task = await _run_extraction(
        session, document_repo, task_repo, entity_repo, doc_entity_repo,
        ref_repo, unresolved_repo, sync_publisher,
...
async def test_extraction_cross_reference_resolution(
    ...
    doc_a = await document_repo.create(
        DocumentCreate(source_url="https://example.com/doc_a.pdf")
    )
    await document_repo.update(doc_a.id, DocumentUpdate(case_number="ÖN 2020-0100"))
    await session.commit()
...
async def test_extraction_unresolved_reference(
    ...
    text_b = (
        "Kyrkoherden överklagade beslut. "
        "Se även avgörandet i ärende ÖN 2099-9999."
    )
    doc_b, _ = await _run_extraction(
        session, document_repo, task_repo, entity_repo, doc_entity_repo,
        ref_repo, unresolved_repo, sync_publisher,
...
async def test_extraction_reconciliation(
    ...
    doc_c = await document_repo.create(
        DocumentCreate(source_url="https://example.com/doc_c.pdf")
    )
    await document_repo.update(doc_c.id, DocumentUpdate(case_number="ÖN 2099-9999"))
    await session.commit()
    doc_c = await document_repo.get_by_id(doc_c.id)
    assert doc_c is not None

    count = await reconcile_references(unresolved_repo, ref_repo, doc_c.id, "ÖN 2099-9999")
    await session.commit()
