...
async def process_parse(
    ...
    if document.gcs_uri is None:
        await task_repo.update_status(
            task.id,
            TaskStatusUpdate(status="failed", error_message=f"Document {document_id} has no stored PDF"),
        )
        await session.commit()
        return
    ...
    try:
        ...
        await document_repo.update(document.id, DocumentUpdate(raw_text=raw_text))
