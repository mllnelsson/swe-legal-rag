...
class TestRetrievePipelineIntegration:
    async def test_retrieve_with_empty_filter_returns_top_chunks(
        ...
        results = await retrieve(
            plan, session, embedding_provider=provider, settings=_settings()
        )
    ...
    async def test_retrieve_reference_traversal_includes_related_document(
        ...
        await _seed_chunk(
            chunk_repo, session, doc1_id, embedding=_unit_vector(0), chunk_index=0
        )
        await _seed_chunk(
            chunk_repo, session, doc2_id, embedding=_unit_vector(0), chunk_index=0
        )
